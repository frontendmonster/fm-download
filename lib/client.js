...
const { mkDirByPathSync } = require('./fs.helper');
...
class Client {
  constructor(format = 'webm', resolution = 720, dir = 'download') {
    ...
    const normilizedDir = path.isAbsolute(dir) ? dir : path.join(__dirname, '..', dir);

    this.baseDir = normilizedDir;
  }
  ...
  async downloadCourseInfo(course) {
    ...
    this.downloadQueue = Object.values(this.courseData.lessonData);

    const dirName = sanitize(this.courseData.title);
    const normilizedDir = path.join(this.baseDir, dirName);

    if (!fs.existsSync(normilizedDir)) {
      mkDirByPathSync(normilizedDir);
    }

    this.dirName = normilizedDir;
  ...
  async downloadLesson(lesson) {
    ...
    const writeStream = read.pipe(fs.createWriteStream(path.join(this.dirName, filename)));
    let total = 0;
    read.on('data', ({ length }) => {
      process.stdout.write(`${chalk.yellow(filename)}: ${(total += length)} bytes downloaded \r`);
    });
